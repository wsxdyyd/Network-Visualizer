...
class Sniffer(Process):
    ...
    def __init__(self, port=None) -> None:
        ...
        if port is None:  # default client/server and pub/sub port
            req_port = 20000
            sub_port = 20001
        else:
            req_port = port
        ...
        self.city_database = pygeoip.GeoIP(
            os.path.join(self.resource_path, "assets/geolocation/GeoLiteCity.dat")
        )  # geolocation database lookup of ip's
        ...
        try:
            ...
            keys = zmq.auth.load_certificate(
                os.path.join(self.resource_path, "configuration/keys/server.key_secret")
            )
            ...
            sys.exit(
                "Issue with ZMQ setup! Try closing any old sniffy.py python processes."
            )
    ...
    def parse_packet(self, packet_header: object, packet_data: object) -> None:
        ...
        timestamp = (
            packet_header.getts()
        )  # returns tuple (seconds, microseconds) of the header
        ...
        if type(ethernet_packet.__dict__["_ProtocolLayer__child"]) == type(
            self.type_ip
        ):  # find better way to check for IP protocol!
            ...
            if (
                ethernet_packet.as_eth_addr(ethernet_packet.get_ether_dhost())
                in self.mac_dict
            ):  # packet inbound (recieved)
                ...
                if (
                    ip_index in self.sniffer_dictionary
                ):  # Previous self.sniffer_dictionary entry found
                    ...
                    self.sniffer_dictionary[ip_index][
                        "data_in"
                    ] = number_of_bytes_of_data  # data inbound
                ...
                if self.sniffer_dictionary[ip_index]["collect_data_flag"] == True:
                    self.sniffer_dictionary[ip_index][
                        "stream_data"
                    ] += ethernet_packet.get_buffer_as_string()
                ...
                if (
                    destination_IP_address in self.sniffer_dictionary
                ):  # Previous self.sniffer_dictionary entry found for outbound destination
                    ...
                    self.sniffer_dictionary[ip_index][
                        "data_out"
                    ] = number_of_bytes_of_data  # data outbound
                    self.sniffer_dictionary[ip_index]["new_packet_boolean"] = True
                    self.sniffer_dictionary[ip_index][
                        "blank_boolean"
                    ] = False  ##rename?
    ...
    def getInterface(self) -> int:
        ...
        for i in interfaces:
            ...
            try:
                ...
                if (
                    interface_dict[0]["addr"] == "00:00:00:00:00:00"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", type=int, help="An interger between 1024-49150 not in use"
    )
