...
class Country_Widget(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.country_color = self.gui_manager.config_variables_dict[
            "Graph Country Color"
        ]
        ...
        self.labels = False
        self.mercator_position = self.initalize_mercator_position(
            kwargs["country_index"]
        )
    ...
    def graph_display(
        ...
        for i, city in enumerate(self.gui_manager.country_dictionary[self.country][1]):
            ...
            ip_draw_angle = angle_between_points(
                country_pos, city_widget.icon_scatter_widget.pos
            )
            total_city_ip_count = (
                len(self.gui_manager.country_dictionary[self.country][1][city]) - 1
            )
            ...
            for n, ip in enumerate(
                self.gui_manager.country_dictionary[self.country][1][city][1:]
            ):
                ...
                final_position = [
                    city_widget.pos[0]
                    + self.ip_radius
                    * cos(ip_draw_angle + radian_ip_phase + angle_slice * n),
                    city_widget.pos[1]
                    + self.ip_radius
                    * sin(ip_draw_angle + radian_ip_phase + angle_slice * n),
                ]
    ...
    def calculate_city_positions(
        ...
        if ip_index % 2 == 0:  # alternate + or -  draw slice
            ip_index *= -1

        if (
            ip_index % 4 == 0 or ip_index % 4 == 1
        ):  # every other odd/even increase radius by 2x
            radius *= 2
    ...
    def toggle_city_widgets(self) -> None:
        ...
        if self.show_city_widgets == True:
            ...
            for city in self.gui_manager.country_dictionary[self.country][1]:
                ...
                if (
                    self.gui_manager.country_dictionary[self.country][1][city][0].exempt
                    == True
                ):
                    continue

                self.gui_manager.country_dictionary[self.country][1][city][
                    0
                ].show = False
    ...
    def toggle_city_labels(self) -> None:
        ...
        if self.city_labels == True:
            ...
            for city in self.gui_manager.country_dictionary[self.country][1]:
                ...
                city_widget = self.gui_manager.country_dictionary[self.country][1][
                    city
                ][0]
    ...
    def make_display_menu(self) -> Scatter:
        ...
        grid_layout = GridLayout(
            cols=2, size_hint=(None, None), size=(dp(300), dp(200)), pos=(dp(10), dp(0))
        )
        ...
        if menu_popup.pos[0] + dp(200) > self.window_x:
            menu_popup.pos = (self.window_x - dp(210), menu_popup.pos[1])

        if menu_popup.pos[1] + dp(200) > self.window_y:
            menu_popup.pos = (menu_popup.pos[0], self.window_y - dp(200))
    ...
    def initalize_mercator_position(self, position: int) -> tuple[float, float]:
        ...
        calculated_x = (
            self.window_center_x / 2
            + floor(((position) % self.gui_manager.mercator_final_country_position))
            * 65
        )
        calculated_y = dp(40) * y_index + 20
    ...
    def update(self, **kwargs) -> None:
        ...
        with self.canvas.before:
            ...
            data_in = map_to_range(
                self.total_data_in, 0, self.gui_manager.country_largest_data_in, 0, 50.0
            )
            data_out = map_to_range(
                self.total_data_out,
                0,
                self.gui_manager.country_largest_data_out,
                0,
                50.0,
            )
        ...
        if graph == True:
            ...
            if self.attach == True:  # update position if widget is attached to parent
                ...
                distance, x_distance, y_distance = distance_between_points(
                    my_computer_position, self.pos
                )  # calculate time-step distance (linear interpolation)
